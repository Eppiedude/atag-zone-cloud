...
The config flow asks for your ATAG account e-mail/username, password, and system or plant ID. Automatic system discovery is not included because no discovery endpoint has been verified. Credentials and the system ID are used only at runtime; never post them in bug reports.
